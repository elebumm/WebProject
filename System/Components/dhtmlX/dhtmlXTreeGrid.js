...
dhtmlXGridObject.prototype._fixAlterCssTGR = function(){ 
		...
		var pid=null;
		var lid=0;
		var css1=this._cssUnEven;
		var css2=this._cssEven;
        for (var i=0; i<this.rowsCol.length; i++){
        	var row=this.rowsCol[i];
        	var lpid=row.parent_id;
        	
        	if (pid!=lpid){
        		if (row.parent_id==lid) level++;
        		else level=this.getLevel(row.idd);
        		pid=lpid;
        		if (this._cssSU){
					css1=this._cssUnEven+" "+this._cssUnEven+"_"+level;
					css2=this._cssEven+" "+this._cssEven+"_"+level;
    			}
    		}
    		lid=row.idd;
            if (this.rowsCol[i].style.display!="none"){
	            if (this.rowsCol[i].className.indexOf("rowselected")!=-1){
    	            if (level%2==1)
        	            this.rowsCol[i].className=css1+" rowselected";
            	    else
                	    this.rowsCol[i].className=css2+" rowselected";
	            }
    	        else{
        	        if (level%2==1)
            	        this.rowsCol[i].className=css1;
                	else
                    	this.rowsCol[i].className=css2;
            	}
        	}
        }
   }
   ...
   */
   ...
   this.setValue = function(valAr){
               ...
               this.cell.parentNode.parent_id = valAr[0];
                 ...
                 this.cell.parentNode.has_kids = valAr[2]||0;
             ...
             /*   if (pRow){
                  var temp_hash = this.grid.loadedKidsHash.get("hashOfParents")
                  if (temp_hash.values._dhx_find(pRow)==-1){
                     temp_hash.put(pRow.idd,pRow);
                     this.grid.loadedKidsHash.put("hashOfParents",temp_hash)
                  }
               }      */
               ...
               node+="<span "+(_isFF?"style='position:relative; top:2px;'":"")+"id='nodeval'>"+valAr[1]+"</span>"
				...
				if (this.grid._onCCH)
					this.grid._onCCH(this.cell.parentNode.idd,this.cell._cellIndex,valAr[1]);
            }
...
dhtmlXGridObject.prototype.expandAll=function(){
        for (var i=0; i<this.rowsCol.length; i++)
            this.expandKids(this.rowsCol[i])
}
/**
...
dhtmlXGridObject.prototype.collapseAll=function(rowId){
        for (var i=0; i<this.rowsCol.length; i++)
            if (this.rowsCol[i].parent_id)
              this.closeItem(this.rowsCol[i].parent_id)
}
