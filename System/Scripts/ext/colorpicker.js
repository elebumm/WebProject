...
Ext.ux.ColorPicker = Ext.extend( Ext.BoxComponent, {
	...
	renderComponent: function() {
		...
		Ext.DomHelper.append( this.cpGetId( 'cCont' ), { 'cls': 'x-cp-colorbox', 'id': this.cpGetId( 'cTransparent' ) }, true ).update( 'Transparent' );
		...
		Ext.get( this.cpGetId( 'cTransparent' ) ).on( 'click', this.setTransparent.createDelegate( this ) );
    ...
    var transparent = this.transparent();		
		...
		Ext.get( this.cpGetId( 'cTransparent' ) ).setStyle({
			'background': '#' + this.rgbToHex( websafe ),
			'color': '#' + this.rgbToHex( wsInvert )
		});
		Ext.getDom( this.cpGetId( 'cTransparent' ) ).title = '#'+this.rgbToHex( websafe );
	...
	setTransparent: function( event, element ) {
		Ext.getCmp( this.cpGetId( 'iHexa' ) ).setValue('transparent');

		this.updateMode = 'hexa';
		var temp = this.rgbToHsv( this.hexToRgb( 'ffffff' ) );
		this.HSV = { h: temp[0], s:temp[1], v:temp[2]};
		this.updateColor();
	},
	selectColor: function( event, element ) {
		this.fireEvent('select', this, Ext.get( element ).getColor( 'backgroundColor', '', '' ));
	  if (Ext.getCmp( this.cpGetId( 'iHexa' ) ).getValue() == 'transparent') {
  		this.fireEvent('select', this, 'transparent');
	  }
	},
	/**
