...
function dhtmlXTreeObject(htmlObject, width, height, rootId){
   ...
   this.tscheck=false;
   this.timgen=true;
   this.beforecheckFuncHandler=0; // ADDED: Milan Zdimal
...
//#}
         ...
         if ((!dhtmlObject._edsbps)||(!dhtmlObject._edsbpsA.length)){
               	window.setTimeout( function(){  dhtmlObject.callEvent("onXLE",[dhtmlObject,parentId]); },1);
...
*/   
   dhtmlXTreeObject.prototype.onCheckBoxClick=function(e)   {
	// ADDED: Milan Zdimal 3/24/2007
	   if (this.treeNod.beforecheckFuncHandler) { 
	      var result = this.treeNod.beforecheckFuncHandler(this.parentObject.id,this.parentObject.checkstate);
...
*     @eventparam: Current checkbox state. 1 - item checked, 0 - item unchecked.
*/
	dhtmlXTreeObject.prototype.setOnBeforeCheckHandler=function(func){  if (typeof(func)=="function") this.beforecheckFuncHandler=func; else this.beforecheckFuncHandler=eval(func); };
...
//#}        
            // ADDED: Extra params to end of request url
            // this.loadXML(src+getUrlSymbol(src)+"uid="+sn+"&id="+this._escape(id));
            // +"&pid="+this._escape(this.getParentId(id))+"&super="+this._escape(this.getUserData(id, "super"))+"&type="+this._escape(this.getUserData(id, "type"))+"&rid="+this._escape(this.getUserData(id, "rid"))
            var xmlstr = src+getUrlSymbol(src)+"uid="+sn+"&id="+this._escape(id)+"&pid="+this._escape(this.getParentId(id))+this.getAllUserData(id);
            try {
            globalDebug(xmlstr);
            } catch(ex) {}
            this.loadXML(xmlstr);
...
/**  
*     @desc: return all user data from target node
*     @type: public
*     @param: itemId - target node id
*     @param: name - key for user data
*     @return: value of user data
...
dhtmlXTreeObject.prototype.getAllUserData=function(itemId){
  ...
  if (!sNode) return;
	...
	for (var property in sNode.userData) {	    
	    if(property.indexOf('t_') != -1) {	        
		    result += '&' + property.replace(/t_/, '') + '=' + this._escape(sNode.userData[property]);
		}
	}		
  return result;  
};
...
// ADDED: Migrated this from my changes from v.1.0

dhtmlXTreeObject.prototype.setSize=function(iWdith, iHeight){
    this.parentObject.style.width = iWidth;
    this.parentObject.style.height = iHeight;
}
