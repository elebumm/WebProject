...
dhtmlXTreeObject.prototype._delayedLoad=function(node,name){
    ...
    // ADDED: New loadXML call	
	 var sid = this.getUserData(findSuper(node.id), "super");
    this.loadXML(this.XMLsource+getUrlSymbol(this.XMLsource)+"id="+this._escape(node.id)+"&super="+this._escape(sid)+"&type="+this._escape(this.getUserData(node.id, "type"))+"&rid="+this._escape(this.getUserData(node.id, "rid")));    
}
